...
- [PropMap/Propedia] 전체 사이트에서 Propedia 링크 `/app/` → `/proppedia/`로 일괄 변경
  - 서버 7개 파일 직접 수정 (정적 파일이므로 즉시 반영)
  - `/app/js/`, `/app/api/` 등 Propedia 웹앱 내부 리소스 경로는 유지 (Nginx alias)
...
- [공통] 부속지번 조회를 건축물대장 API → complex_parcels DB로 대체 (search_by_jibun, search_by_bdmgtsn)
- [Propedia] 지도 클릭 시 단지 매칭되면 단지명+세대수 표시 (앱: ComplexInfo DTO, 웹: search-map.html)
- [공통] 통합검색에서 지번→단지 자동 매칭 — "신천동 20" → 파크리오 최상위 노출 (search_unified.py)
- [데이터] 50세대+ 부속지번 배치 완료 (6,572건), 20-49세대 크론 실행 대기 중

## 2026-04-29: 집합건축물 대표지번 자동 전환 + 지도 UX 개선

- [Propedia/공통] 지도 클릭 시 집합건축물 부속지번 → 대표지번 자동 전환 (서버 API 수정, 앱/웹 변경 없음)
- [공통] 부속지번 일괄 등록 배치 스크립트 (`batch_attached_jibun.py`) — 50세대+ 실행, 20-49세대 크론 대기
- [Propedia] 지도 뒤로가기 시 마지막 조회 위치 유지 (앱: provider 좌표 보존, 웹: sessionStorage)
...
- [인프라] daily_backup.sh 실행 권한 누락(`0644`) → `chmod +x` 복구 (4/9 이후 15일간 백업 미실행)
- [인프라] backup_notify.py 신규 생성 — 백업 성공/실패/미실행 3가지 이메일 알림
  - .env 파싱 시 따옴표 미제거 버그 수정 (일간보고 config.py와 동일 로직 적용)
- [인프라] cron fallback 추가 — 스크립트 실행 자체 실패 시에도 not_started 알림 발송
- [인프라] 서버 로컬 3일 + Google Drive 14일 보관 정책 정상 동작 확인
