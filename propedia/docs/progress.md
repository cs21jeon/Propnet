...
- **PNU off-by-one 버그 수정** (home_screen.dart `_navigateToResultDirect`):
  - PNU `[법정동10][지목1][본번4][부번4]`에서 지목 1자리를 건너뛰지 않아 본번/부번이 1자리씩 밀림
  - "독산동 939-11" 검색 시 "1093-9001"로 표시되던 문제
  - `substring(10,14)` → `substring(11,15)`, `substring(14,18)` → `substring(15,19)`
  - 웹은 04-17에 수정 완료, 앱만 미반영이었음
- **지도 안내 문구 추가** (home_screen.dart): 아무것도 선택되지 않은 상태에서 "지도에서 직접 선택하세요" 안내 표시 (반투명 배지, IgnorePointer로 탭 방해 없음)
