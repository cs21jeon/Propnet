#!/usr/bin/env python3
"""Patch propsheet.py: Update search-map endpoint to support 3 property types"""
import re
import sys
...
# Find search_map_db function
old_pattern = r"@bp\.route\('/api/propsheet/search-map', methods=\['POST'\]\)\ndef search_map_db\(\):.*?return jsonify\(\{\s*'map_html': map_html,\s*'markers': markers,\s*'count': len\(markers\),\s*'statistics':.*?\}\s*\)"

match = re.search(old_pattern, content, re.DOTALL)
if not match:
    print("ERROR: Could not find search-map endpoint")
    sys.exit(1)

new_func = r"""@bp.route('/api/propsheet/search-map', methods=['POST'])
def search_map_db():
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if property_type == 'danil':
                    ...
                    if price_val and price_cond != 'all':
                        conditions.append('"매가(만원)" ' + ('>= %s' if price_cond == 'above' else '<= %s'))
                    ...
                    if inv_val and inv_cond != 'all':
                        conditions.append('"실투자금" ' + ('>= %s' if inv_cond == 'above' else '<= %s'))
                    ...
                    if yield_val and yield_cond != 'all':
                        conditions.append('"융자제외수익률(%%)" ' + ('>= %s' if yield_cond == 'above' else '<= %s'))
                    ...
                    if area_val and area_cond != 'all':
                        conditions.append('"토지면적(㎡)" ' + ('>= %s' if area_cond == 'above' else '<= %s'))
                    ...
                    if dep_val and dep_cond != 'all':
                        conditions.append('"보증금(만원)" ' + ('>= %s' if dep_cond == 'above' else '<= %s'))
                    ...
                    if rent_val and rent_cond != 'all':
                        conditions.append('"월세(만원)" ' + ('>= %s' if rent_cond == 'above' else '<= %s'))
                ...
                for row in cur.fetchall():
                    ...
                    if property_type == 'danil':
                        ...
                        price_display = f"{price_eok:.1f}억원" if price_eok >= 1 else (f"{price_num:.0f}만원" if price_num else "가격미정")
                        land_area = to_float(row.get('토지면적(㎡)'))
                        land_str = f"{land_area/3.3058:.0f}평 ({land_area:.1f}㎡)" if land_area else "면적미정"
                        floors = str(row.get('층수', '') or '')
                        usage = str(row.get('주용도', '') or '')
                        popup = (
                            f'<button class="close-btn">&times;</button>'
                            ...
                            f'<button class="btn-detail" data-record="{record_id}" data-dbid="39">상세내역보기</button>'
                        ...
                        if txn == '월세':
                            ...
                            pe = price_num / 10000
                            pd = f"매매 {pe:.1f}억" if pe >= 1 else f"매매 {price_num:.0f}만"
                        popup = (
                            f'<button class="close-btn">&times;</button>'
                            f'<div class="address">{addr_esc}</div>'
                            f'<div class="info-row">{pd}</div>'
                            f'<div class="info-row">전용: {excl/3.3058:.0f}평 ({excl:.1f}㎡)</div>' if excl else ''
                            f'<div class="info-row">방: {int(rooms)}개</div>' if rooms else ''
                            f'<button class="btn-detail" data-record="{record_id}" data-dbid="38">상세내역보기</button>'
                            f'<button class="btn-consult" data-address="{addr_esc}">이 매물 문의하기</button>'
                        )
                        markers.append({
                            'lat': to_float(row['coordinates_lat']),
                        ...
                        popup = (
                            ...
                            + (f'<div class="info-row">방: {int(rooms)}개</div>' if rooms else '')
                            + f'<button class="btn-detail" data-record="{record_id}" data-dbid="43">상세내역보기</button>'
                            f'<button class="btn-consult" data-address="{addr_esc}">이 매물 문의하기</button>'
                        )
                        markers.append({
                            'lat': to_float(row['coordinates_lat']),
...
print(f"SUCCESS: Replaced search-map endpoint ({match.start()}-{match.end()})")
