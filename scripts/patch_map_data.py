#!/usr/bin/env python3
"""Patch propsheet.py: Replace map-data endpoint to support 3 property types"""
import re
import sys
...
# Find the map-data endpoint
old_func_pattern = r"@bp\.route\('/api/propsheet/map-data'.*?return jsonify\(\{'success': True, 'markers': markers, 'total': len\(markers\), 'agent': agent\}\)"

new_func = '''@bp.route('/api/propsheet/map-data', methods=['GET'])
def get_map_data():
    ...
    txn_param = request.args.get('txn', '')  # 매매,전세,월세 필터
    ...
    # 에이전트 정보 (금토끼부동산 워크스페이스 → agent_id)
    agent = None
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
...
if not match:
    print("ERROR: Could not find map-data endpoint pattern")
    sys.exit(1)
...
print(f"SUCCESS: Replaced map-data endpoint ({match.start()}-{match.end()})")
print(f"Original backed up to {FILE}.bak")
