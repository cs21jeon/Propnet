#!/usr/bin/env python3
"""Step 2: 매칭 + SCP 업로드 + DB 업데이트"""
import os, re, json, subprocess, uuid
...
print(f"매칭: {len(plan)}건 레코드, {total_files}개 파일\n")
...
for i, item in enumerate(plan):
    ...
    if files_info:
        ...
        print(f"[{i+1}/{len(plan)}] id={rid} {item['addr']}: {len(files_info)}개")

# 5. JSON을 서버에 전송
print(f"\nDB 업데이트 준비 ({len(upload_data)}건)...")
