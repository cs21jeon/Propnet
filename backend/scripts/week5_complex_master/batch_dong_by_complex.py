...
# -*- coding: utf-8 -*-
"""
building_dong_geometry 배치 수집 — 단지(complex) 단위 최적화.

기존 PNU 단위 대비 장점:
  - center_lat 활용 → Data API 호출 불필요 (API 호출 절반)
  - 같은 단지의 다중 PNU를 1회 WFS로 커버
  - 26,000 단지 × 1 WFS = ~2.2시간 완료

사용:
  python batch_dong_by_complex.py --limit 30000 --rate-limit 0.3
  python batch_dong_by_complex.py --dry-run
"""
import argparse
import json
import logging
...
log = logging.getLogger('batch_dong_complex')
...
def collect_for_complex(complex_pk, center_lat, center_lon, pnu_list, apikey, conn):
    """
    1개 단지에 대해 center_lat 기반 BBOX WFS 호출 → 동별 좌표 저장.
    pnu_list: 이 단지에 속한 PNU 목록 (필터링용).
    반환: 저장된 동 수.
    """
    ...
    try:
        ...
        log.debug('WFS 실패 complex=%s: %s', complex_pk, e)
        return 0

    if not features:
        return 0

    # PNU prefix set (15자리: 법정동+산+본번)
    ...
    for (bd_mgt_sn, d_pnu, dong_nm, bld_nm, lat, lon, geom, grnd_flr, archarea) in dongs:
        try:
            ...
            log.debug('저장 실패 %s: %s', bd_mgt_sn, e)
            conn.rollback()
            continue
    conn.commit()
    return saved
...
def main():
    ...
    apikey = get_apikey()
    if not apikey:
        log.error('VWORLD_APIKEY 없음')
        return

    conn = get_conn()
    cur = conn.cursor()

    # 1단계: center_lat 있는 50세대+ 단지 전체 (가벼운 쿼리)
    cur.execute("""
    ...
    # 2단계: 이미 처리된 단지 제외 (building_dong_geometry에 PNU가 1개라도 있으면 스킵)
    already_done = set()
    cur.execute("""
    ...
    log.info('수집 대상 단지: %d개 (min_hh=%d)', len(targets), args.min_household)
    ...
    for i, (cpk, clat, clon, pnu_list) in enumerate(targets):
        try:
            saved = collect_for_complex(cpk, clat, clon, pnu_list, apikey, conn)
            total_saved += saved
            if saved > 0:
                success_count += 1
                fail_streak = 0
        ...
        if (i + 1) % 200 == 0:
            ...
            log.info('진행: %d/%d 단지, 동저장 %d, 성공 %d (%.0f초)',
                     i + 1, len(targets), total_saved, success_count, elapsed)
    ...
    elapsed = time.time() - start
    log.info('완료: %d/%d 단지, 동저장 %d, 성공 %d (%.0f초)',
             len(targets), len(targets), total_saved, success_count, elapsed)
