...
> 최종 업데이트: 2026-03-23 (서버 버그 수정 + 음성 요약 UI 리디자인 + 요금제 동적 로딩)
...
59. **채팅방 삭제 버그 수정** (usage_logs FK 제약 위반으로 500 에러 → Room.delete()에서 usage_logs 선삭제) ✅ **신규**
60. **음성파일 요약 API 배포** (routes_audio.py 서버 미배포 → 배포 + app.py 라우트 등록) ✅ **신규**
61. **API 응답 파싱 안전성 강화** (_handleResponse에서 비-JSON 응답 시 FormatException 대신 의미있는 에러 메시지) ✅ **신규**
62. **room_members 상태값 수정** (요약 쿼리 조건 'approved' → IN ('approved', 'active') 호환) ✅ **신규**
63. **음성 요약 화면 2단계 구조 리디자인** (채팅방 리스트 → 방 선택 → 해당 방 요약 카드 목록, 원본 텍스트 제거) ✅ **신규**
64. **요약 카드 overflow 수정** (이름/전화번호에 Flexible + TextOverflow.ellipsis 적용) ✅ **신규**
65. **메인 메뉴 텍스트 개선** (업무 채팅방 가기 / 음성 요약 보러가기 / 음성 및 업무 파일 확인) ✅ **신규**
66. **요금제 안내 동적 로딩** (하드코딩 제거 → 서버 API에서 실시간 조회, 일반/Agent 자동 분리) ✅ **신규**
67. **결제 약관 가격 수정** (terms.dart 요금 DB와 일치: 4,900/19,900/29,900/79,900원) ✅ **신규**
68. **Agent Regular 시간 수정** (DB minutes_included 1800→60, 1시간 팩으로 정정) ✅ **신규**
