...
### Phase 6+++++ ✅ Proptalk 동기화 수정 + 필터 강화 + 수식 필터 (2026-03-25)

**Proptalk 동기화 버그 수정**:
- [x] `goldenrabbit_talk` 테이블에 `proptalk_audio_id` 컬럼 추가 (누락으로 auto-sync 실패하던 버그)
- [x] 기존 34건 레코드에 voiceroom audio_id 매핑 (original_filename 기준)
- [x] 필드명 변경: `날짜` → `녹음날짜` (record_date는 통화일이므로 명확한 이름으로)

**필터 기능 — 필드 타입별 조건 분리 (Airtable 스타일)**:
- [x] 텍스트 필드: 포함, 포함하지 않음, 같음, 같지 않음, 비어있음, 비어있지 않음
- [x] 숫자 필드: 같음, 같지 않음, 보다 큼, 보다 작음, 이상, 이하, 비어있음, 비어있지 않음
- [x] 날짜 필드: 같음, 같지 않음, 이전, 이후, 이전이거나 같음, 이후이거나 같음, 비어있음, 비어있지 않음
- [x] 날짜 필드에 `<input type="date">` 날짜 피커 UI
- [x] 선택 필드: 하나라도 포함, 모두 포함하지 않음, 같음, 비어있음, 비어있지 않음 (기존 유지)
- [x] 체크박스 필드: 같음, 비어있음, 비어있지 않음
- [x] 새 operator: `is_before`, `is_after`, `is_on_or_before`, `is_on_or_after` (백엔드 SQL 지원)
- [x] `is_empty`/`is_not_empty` 백엔드 버그 수정 (value 빈 문자열 시 스킵되던 문제)

**수식(formula) 필드 결과값 필터링**:
- [x] `filterableColumns`에서 수식 필드 제외 해제 — 필터 드롭다운에 수식 필드 표시
- [x] 수식 필드 operator: 포함, 같음, 같지 않음, 보다 큼, 보다 작음, 이상, 이하, 비어있음, 비어있지 않음
- [x] 백엔드: 수식 필드 필터 시 SQL 표현식을 WHERE 절에 삽입 (숫자 비교 `::numeric`, 텍스트 비교 `::text` 캐스팅)

### Phase 6++++++ ✅ 추천매물 DB 전환 + 이미지 최적화 + Top 버튼 (2026-03-25)

**추천매물 Airtable JSON → PropSheet DB 뷰 전환**:
- [x] `GET /api/propsheet/category-properties?view_id={id}` 새 API — PropSheet 뷰의 filter_config/sort_config 적용하여 DB 직접 조회
- [x] `list_properties()` + `get_view()` 재사용, Airtable 호환 응답 형태 유지
- [x] index.html 버튼 `data-filter`를 PropSheet 뷰 ID(70/71/72)로 변경
- [x] `loadCategoryProperties()` API URL을 새 엔드포인트로 변경
- [x] `categoryDescriptions` 키를 PropSheet 뷰 ID로 변경
- [x] 대지면적 라벨에 `토지면적(㎡)` 값 사용 (3개 뷰 모두)
- [x] 레거시 `/api/category-properties` (propnet_api.py) 유지 (Propedia 앱 호환)

**이미지 로딩 최적화**:
- [x] API 응답에 이미지 URL 포함 시 `check-image` API 호출 없이 바로 표시
- [x] URL 없는 매물만 기존 `loadPropertyImage` 3단 폴백 사용
- [x] 매물 N개당 N회 → 0회 API 호출로 서버 부하 대폭 감소

**UI 개선**:
- [x] Scroll-to-Top 버튼 — 1페이지 이상 스크롤 시 우하단에 ↑ 버튼 표시, 부드러운 스크롤

**데이터 수정**:
- [x] 독산동 969-6 대표사진/이미지 연결 (Airtable 백업 이미지 심볼릭 링크)

### Phase 6+++++++ ✅ 수식 엔진 + 뷰 필터 + 파일 한도 (2026-03-30)

**단일부동산 수식(formula) 11개 등록**:
- [x] 독립 수식 6개: 주구조값, 연식값, 공시가(만원), 토지가치(만원), 10년간수익(만원), 10년후토지가치(만원)
- [x] 의존 수식 5개 (인라인 전개): 건물가치(만원), 10년후건물가치(만원), 현재가치지수(%), 10년가치지수(%), 감정비교
- [x] 건물가치 조건 수정: 기존 트리거 `공시가 < 주구조값` → Airtable 원본 `연식값 >= 주구조값`

**뷰 필터 연동 개선**:
- [x] `saveCurrentView()` payload에 `database_id` 누락 → 일반 editor 세션에서 "Database not specified" 에러 수정
- [x] 집합부동산(db_id=38) "등록" 뷰 filter_config 복구 (빈 배열 → 현황=등록)
- [x] 셀 편집 후 뷰 필터 불일치 시 0.3초 fade-out으로 행 자동 제거 (`_matchesCurrentFilter` 클라이언트 체크, 서버 호출 없음)

**파일 저장 한도**:
- [x] `MAX_DB_SIZE` 1GB → 2GB per database (database.py)
