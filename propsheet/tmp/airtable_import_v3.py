#!/usr/bin/env python3
"""Airtable → Propsheet import (v3 - fixed)"""
...
def convert(v):
    ...
    if isinstance(v, list):
        ...
        if isinstance(v[0], dict):  # attachment
...
def main():
    print('=== Airtable Import v3 ===', flush=True)
    records = fetch_all()
    ...
    for rec in records:
        try:
            ...
            cur.execute(f'INSERT INTO "{TABLE_NAME}" ({cs}) VALUES ({ph})', vals)
            inserted += 1
    ...
    cur.close()
    conn.close()
    print(f'\n=== Done! {inserted} records imported ===', flush=True)
