#!/usr/bin/env python3
"""Fix trigger: remove REPLACE for map URL + update existing data + formula"""
import sys, os
...
with get_db_connection() as conn:
    with conn.cursor() as cur:
        # Step 1: Fix trigger — remove REPLACE, keep spaces
        cur.execute("""
            ...
            BEGIN
                ...
                NEW."순매가(만원)" := COALESCE(NEW."매가(만원)", 0) - COALESCE(NEW."보증금(만원)", 0) - COALESCE(NEW."융자(만원)", 0);
            ...
            $$ LANGUAGE plpgsql;
        """)
        print("1. Trigger updated (REPLACE removed)")
        ...
        # Step 3: Fix field_definitions formula
        cur.execute("""
            UPDATE field_definitions
            SET formula = '''https://map.kakao.com/link/search/'' || "지번 주소"'
            WHERE field_name = '지도' AND database_id = 1
        """)
        print(f"3. Updated formula definition")
