...
if 'clone_workspace' not in content:
    # Also need create_workspace import
    if 'create_workspace' not in content:
...
def api_clone_workspace(slug):
    ...
    try:
        ...
        # Clone all databases in this workspace
...
if 'cloneWorkspace' not in html:
    old_btn = '''<button class="edit-btn" @click.stop="openEditWorkspaceModal(workspace)" title="워크스페이스 편집">
    ...
    new_btn = '''<button class="edit-btn" @click.stop="cloneWorkspace(workspace)" title="워크스페이스 복제">
    ...
    print("3. Added clone button to UI")

import re
html = re.sub(r'workspaces\.js\?v=\w+', 'workspaces.js?v=20260318b', html)
