#!/usr/bin/env python3
"""Airtable → Propsheet import (v4 - sql.Identifier fix)"""
...
def main():
    print('=== Airtable Import v4 ===', flush=True)
    records = fetch_all()
    ...
    for col, ctype in [('airtable_id','VARCHAR(20)'),('record_id','VARCHAR(64)'),('database_id','INTEGER')]:
        if col not in existing:
            cur.execute(sql.SQL('ALTER TABLE {} ADD COLUMN IF NOT EXISTS {} ' + ctype).format(
                sql.Identifier(TABLE_NAME), sql.Identifier(col)))
    for fname, ftype in all_fields.items():
        if fname not in existing:
            cur.execute(sql.SQL('ALTER TABLE {} ADD COLUMN IF NOT EXISTS {} ' + ftype).format(
                sql.Identifier(TABLE_NAME), sql.Identifier(fname)))
    conn.commit()
    ...
    # Insert using sql.SQL to handle % in column names
