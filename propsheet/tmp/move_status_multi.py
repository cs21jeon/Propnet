#!/usr/bin/env python3
"""Move 현황 → 광고 for 집합부동산 (same logic as 단일부동산)"""
import sys, os
...
with get_db_connection() as conn:
    with conn.cursor() as cur:
        ...
        for row_id, status in rows:
            ...
            if ad_parts:
                new_status = status_parts[0] if status_parts else '등록'
            else:
                new_status = status_parts[0] if status_parts else '등록대기'
        ...
        cur.execute(f'SELECT "광고", COUNT(*) FROM "{TABLE}" WHERE "광고" IS NOT NULL GROUP BY "광고" ORDER BY COUNT(*) DESC LIMIT 10')
        print('\n=== 변환 후 광고 ===')
        for r in cur.fetchall():
            print(f'  {r[0]} → {r[1]}건')
