...
- 상세보기에서 광고(자동완성)과 중복되는 info rows 제거 (토지면적, 연면적, 층수, 주용도, 용도지역, 사용승인일, 인접역 등)
- 단일부동산만 실투자금 + 융자제외수익률 표시, 집합/부분은 광고(자동완성)만
- 서버 8개 파일 전수 반영: propmap/{map,index}.html, goldenrabbit/{map,index}.html, propnet/map.html, _template/map.html, 루트 {map,index}.html
- CLAUDE.md에 "동일 UI를 서빙하는 복수 경로" 표 추가 — 상세보기 수정 시 전수 확인 필수 가이드
...
- `ai_search_service.py`에 `AI_BLOCKED_COLUMNS` 상수 추가 (frozenset, 20개 컬럼)
  - 소유자 PII: 소유자명, 소유주연락처, 소유자주소, 소유자생년월일
  - 내부 운영: 비공개메모, 홍보문구, 상단/하단고정 홍보문구, 사진링크, 건축물대장, 네이버, 문의, 대표, 주인세대, 주인거주, 사진필요, SH가능
  - 시스템: fields_hash, synced_at
- `_sanitize_row()` 방어 함수 추가 — Stage 1 결과에서 차단 컬럼 자동 제거
  - Stage 1 SQL이 이미 허용 컬럼만 SELECT하지만, 향후 SQL 수정 시 실수 방어용 2차 레이어
- 기존 LLM 프롬프트 레벨 가드(refuse intent)는 유지 — 데이터 레이어 + 프롬프트 레이어 이중 방어 구조
